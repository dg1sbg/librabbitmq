...
#undef RABBITMQ_C_HAS_SYSLOG
#define inline __inline
#define RABBITMQ_EXPORT __declspec(dllexport)

#else

#define RABBITMQ_C_HAS_SYSLOG 1
#include <syslog.h>
#define RABBITMQ_EXPORT
...
typedef void (*pfnLogFn_t)(char *pcFile, int nLine, int nPrio, char *pcFormat, ... );
...
RABBITMQ_EXPORT extern char const *amqp_version(void);

RABBITMQ_EXPORT extern void init_amqp_pool(amqp_pool_t *pool, size_t pagesize);
RABBITMQ_EXPORT extern void recycle_amqp_pool(amqp_pool_t *pool);
RABBITMQ_EXPORT extern void empty_amqp_pool(amqp_pool_t *pool);

RABBITMQ_EXPORT extern void *amqp_pool_alloc(amqp_pool_t *pool, size_t amount);
RABBITMQ_EXPORT extern void amqp_pool_alloc_bytes(amqp_pool_t *pool, size_t amount, amqp_bytes_t *output);

RABBITMQ_EXPORT extern amqp_bytes_t amqp_cstring_bytes(char const *cstr);
RABBITMQ_EXPORT extern amqp_bytes_t amqp_bytes_malloc_dup(amqp_bytes_t src);
RABBITMQ_EXPORT extern amqp_bytes_t amqp_bytes_malloc(size_t amount);
RABBITMQ_EXPORT extern void amqp_bytes_free(amqp_bytes_t bytes);

RABBITMQ_EXPORT extern amqp_connection_state_t amqp_new_connection(void);
RABBITMQ_EXPORT extern int amqp_get_sockfd(amqp_connection_state_t state);
RABBITMQ_EXPORT extern void amqp_set_sockfd(amqp_connection_state_t state,
			    int sockfd);
RABBITMQ_EXPORT extern int amqp_tune_connection(amqp_connection_state_t state,
				int channel_max,
				int frame_max,
				int heartbeat);
RABBITMQ_EXPORT extern int amqp_get_channel_max(amqp_connection_state_t state);
RABBITMQ_EXPORT extern int amqp_destroy_connection(amqp_connection_state_t state);

RABBITMQ_EXPORT extern int amqp_handle_input(amqp_connection_state_t state,
			     amqp_bytes_t received_data,
			     amqp_frame_t *decoded_frame);

RABBITMQ_EXPORT extern amqp_boolean_t amqp_release_buffers_ok(amqp_connection_state_t state);

RABBITMQ_EXPORT extern void amqp_release_buffers(amqp_connection_state_t state);

RABBITMQ_EXPORT extern void amqp_maybe_release_buffers(amqp_connection_state_t state);

RABBITMQ_EXPORT extern int amqp_send_frame(amqp_connection_state_t state,
			   amqp_frame_t const *frame);
RABBITMQ_EXPORT extern int amqp_send_frame_to(amqp_connection_state_t state,
			      amqp_frame_t const *frame,
			      amqp_output_fn_t fn,
			      void *context);

RABBITMQ_EXPORT extern int amqp_table_entry_cmp(void const *entry1, void const *entry2);

RABBITMQ_EXPORT extern int amqp_open_socket(char const *hostname, int portnumber);

RABBITMQ_EXPORT extern int amqp_send_header(amqp_connection_state_t state);
RABBITMQ_EXPORT extern int amqp_send_header_to(amqp_connection_state_t state,
			       amqp_output_fn_t fn,
			       void *context);

RABBITMQ_EXPORT extern amqp_boolean_t amqp_frames_enqueued(amqp_connection_state_t state);

RABBITMQ_EXPORT extern int amqp_simple_wait_frame(amqp_connection_state_t state,
				  amqp_frame_t *decoded_frame);

RABBITMQ_EXPORT extern int amqp_simple_wait_method(amqp_connection_state_t state,
				   amqp_channel_t expected_channel,
				   amqp_method_number_t expected_method,
				   amqp_method_t *output);

RABBITMQ_EXPORT extern int amqp_send_method(amqp_connection_state_t state,
			    amqp_channel_t channel,
			    amqp_method_number_t id,
			    void *decoded);

RABBITMQ_EXPORT extern amqp_rpc_reply_t amqp_simple_rpc(amqp_connection_state_t state,
					amqp_channel_t channel,
...
RABBITMQ_EXPORT extern amqp_rpc_reply_t amqp_login(amqp_connection_state_t state,
				   char const *vhost,
				   int channel_max,
				   int frame_max,
				   int heartbeat,
...
RABBITMQ_EXPORT extern struct amqp_channel_open_ok_t_ *amqp_channel_open(amqp_connection_state_t state,
							 amqp_channel_t channel);

struct amqp_basic_properties_t_;
RABBITMQ_EXPORT extern int amqp_basic_publish(amqp_connection_state_t state,
			      amqp_channel_t channel,
...
RABBITMQ_EXPORT extern amqp_rpc_reply_t amqp_channel_close(amqp_connection_state_t state,
					   amqp_channel_t channel,
					   int code);
RABBITMQ_EXPORT extern amqp_rpc_reply_t amqp_connection_close(amqp_connection_state_t state,
					      int code);

RABBITMQ_EXPORT extern struct amqp_exchange_declare_ok_t_ *amqp_exchange_declare(amqp_connection_state_t state,
								 amqp_channel_t channel,
								 amqp_bytes_t exchange,
								 amqp_bytes_t type,
...
RABBITMQ_EXPORT extern struct amqp_queue_declare_ok_t_ *amqp_queue_declare(amqp_connection_state_t state,
							   amqp_channel_t channel,
							   amqp_bytes_t queue,
							   amqp_boolean_t passive,
							   amqp_boolean_t durable,
...
RABBITMQ_EXPORT extern struct amqp_queue_delete_ok_t_ *amqp_queue_delete(amqp_connection_state_t state,
							 amqp_channel_t channel,
							 amqp_bytes_t queue,
							 amqp_boolean_t if_unused,
							 amqp_boolean_t if_empty);

RABBITMQ_EXPORT extern struct amqp_queue_bind_ok_t_ *amqp_queue_bind(amqp_connection_state_t state,
						     amqp_channel_t channel,
						     amqp_bytes_t queue,
						     amqp_bytes_t exchange,
						     amqp_bytes_t routing_key,
...
RABBITMQ_EXPORT extern struct amqp_queue_unbind_ok_t_ *amqp_queue_unbind(amqp_connection_state_t state,
							 amqp_channel_t channel,
							 amqp_bytes_t queue,
							 amqp_bytes_t exchange,
...
RABBITMQ_EXPORT extern struct amqp_basic_consume_ok_t_ *amqp_basic_consume(amqp_connection_state_t state,
							   amqp_channel_t channel,
							   amqp_bytes_t queue,
...
RABBITMQ_EXPORT extern int amqp_basic_ack(amqp_connection_state_t state,
			  amqp_channel_t channel,
			  uint64_t delivery_tag,
			  amqp_boolean_t multiple);

RABBITMQ_EXPORT extern amqp_rpc_reply_t amqp_basic_get(amqp_connection_state_t state,
          amqp_channel_t channel,
          amqp_bytes_t queue,
          amqp_boolean_t no_ack);

RABBITMQ_EXPORT extern struct amqp_queue_purge_ok_t_ *amqp_queue_purge(amqp_connection_state_t state,
            amqp_channel_t channel,
            amqp_bytes_t queue,
            amqp_boolean_t no_wait);

RABBITMQ_EXPORT extern struct amqp_tx_select_ok_t_ *amqp_tx_select(amqp_connection_state_t state,
						   amqp_channel_t channel);

RABBITMQ_EXPORT extern struct amqp_tx_commit_ok_t_ *amqp_tx_commit(amqp_connection_state_t state,
						   amqp_channel_t channel);

RABBITMQ_EXPORT extern struct amqp_tx_rollback_ok_t_ *amqp_tx_rollback(amqp_connection_state_t state,
						       amqp_channel_t channel);
 ...
 */
RABBITMQ_EXPORT extern amqp_boolean_t amqp_data_in_buffer(amqp_connection_state_t state);
 ...
 * instance.
 */
RABBITMQ_EXPORT extern amqp_rpc_reply_t amqp_get_rpc_reply(amqp_connection_state_t state);
 ...
 * for freeing it.
 */
RABBITMQ_EXPORT extern char *amqp_error_string(int err);

/*
 * "Opens" this library and initializes logging.
 */
RABBITMQ_EXPORT extern void amqp_lib_open( pfnLogFn_t pLogFn, int nLogLevel, int nFacility, char *pcName );

/*
 * "Close" this library and closes logging.
 */
RABBITMQ_EXPORT extern void amqp_lib_close(void);

/*
 * Error handling and loggimng functions
 */

RABBITMQ_EXPORT extern void amqp_log( char *pcFile, int nLine, int nPrio, char *pcFormat, ... );
extern void amqp_log_error( char *pcFile, int nLine, int nPrio, int nError, char *pcFormat, ...);
RABBITMQ_EXPORT extern void amqp_clear_error( void );
RABBITMQ_EXPORT extern int  amqp_get_error(void);

/*
 * Returns the name of this library
 */
RABBITMQ_EXPORT extern char *amqp_libname(void);

/*
 * Returns 1 if this library has been "opened" with a call to
 * amqp_lib_open() else it returns 0.
 */
RABBITMQ_EXPORT extern unsigned int  amqp_libopened(void);
